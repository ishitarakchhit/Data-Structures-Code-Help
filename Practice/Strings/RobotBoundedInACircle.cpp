...
bool isRobotBounded(string instructions) {
        ...
        int a = 0;
        int b = 0;                    //coordinates of starting position
        for(int i=0; i<instructions.length(); i++){
            char step = instructions[i];       //G L R
            if(step == 'G'){
                if(direction == 'U') b++;
        ...
        //----! mistake: did not correctly think of these conditions 
        if(a == 0 && b == 0 ) return true;                
        else if( direction == 'U') return false;
        return true;
    }
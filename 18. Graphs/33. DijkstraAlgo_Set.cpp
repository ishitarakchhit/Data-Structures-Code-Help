...
vector <int> dijkstra(int V, vector<vector<int>> adj[], int S)
    {
        ...
        //pushing the source node with distance 0
        st.insert({0, S});
        
        //vector to track distances of all nodes from source node
        ...
        while(!st.empty()){
            ...
            for(auto &x: adj[node]){
                int currDist = dist + x[1];
                int currNode = x[0];
                
                if(distance[currNode] > currDist){
                    if(distance[currNode] != INT_MAX){   //which means that this node has already been visited which has worse distance than the current so it will create redundtant iteations
